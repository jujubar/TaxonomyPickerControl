//Modifying to refer what is returned from K2 Taxonomy SMO instead of standard SharePoint Objects
// This involves changing anything that is 'raw' into something that you can get from K2

// Adding resouces class directly here
...
(function (CAMControl) {
    ...
    $.extend(Term.prototype, {
        ...
        clone: function () {
            return new Term(this.Id, this.Name, this.PathOfTerm);
        },
        //converts a term to an html tree node
        toHtmlLabel: function () {
            var addlClass = (this.Children.length > 0) ? 'collapsed' : '';
            return $('<li class="cam-taxpicker-treenode-li"><div class="cam-taxpicker-treenode"><div class="cam-taxpicker-expander ' + addlClass + '"></div><img src="' + <%= WebResource("TaxonomyPickerControl.EMMTerm.png")%> + '" alt=""/><span class="cam-taxpicker-treenode-title"  data-item="' + this.Name + '|' + this.Id + '">' + this.Name + '</span></div></li>');
        }
    });
    //********************** END Term Class **********************
    ...
    $.extend(TermSet.prototype, {
        ...
        initialize: function () {
            ...
            var termStore = "toBeDefinated"; //Changing this for SMO


            //get the termset based on the properties of the termset
            // Only the default will be used
            if (this.Id != null)
                this.RawTermSet = termStore.getTermSet(this.Id); //get termset by id - Should go straight to where I am storing values
            ...
            //this.RawTerms = this.RawTermSet.getAllTerms(); // Need to get this on load, maybe store in control?
            // Will bypass all RawTerms to insert data directly into flat terms


            this.termsLoadedSuccess();
            ...
            for (var i = 0; i < parts.length - 1; i++) {
                for (var j = 0; j < termList.length; j++) {
                    if (parts[i] == termList[j].Name) {
                        term = termList[j];
                        termList = term.Children; // Need to change this line, 
                        break;
                    }
                }
            }
    ...
    function TaxonomyPicker(control, options, context, changeCallback) {
        ...
        // this._context = context; //Context passed in from control - Will not be using anymore
    ...
    $.extend(TaxonomyPicker.prototype, {
                    ...
                    if (suggestions.length > 0) {
                        $(suggestions).each(Function.createDelegate(this, function (i, e) {
                            if (i < this._maxSuggestions) {
                                ...
                                // Commenting out the localization, and hardcoding this to default label name
        ...
        dialogNewTermClicked: function (event) {
            ...
            this._dlgNewNode = $('<li class="cam-taxpicker-treenode-li newNode"></li>').append($('<div class="cam-taxpicker-treenode"></div>').append('<div class="cam-taxpicker-expander"></div><img src="' + <%= WebResource("TaxonomyPickerControl.EMMTerm.png")%> + '" alt=""/>').append(this._dlgNewNodeEditor));
        ...
        showPickerDialog: function (event) {
            ...
            if (!this.TermSet.TermsLoaded) {
                ...
                if (this._dialog == null) {
                    ...
                    //build dialog body
                    var dlgSubheader = $('<div class="cam-taxpicker-dialog-content-subheader"><img class="cam-taxpicker-dialog-content-subheader-img" src="' + <%= WebResource("TaxonomyPickerControl.EMMDoubleTag.png")%> + '" alt="" /></div>');
                    ...
                    document.getElementById('rootNode').innerHTML =
                                       '<li class="cam-taxpicker-treenode-li">' +
                                           '<div class="cam-taxpicker-treenode">' +
                                               ...
                                               '<img src="' + <%= WebResource("TaxonomyPickerControl.EMMTermSet.png")%> + '" alt=""/>' +
        ...
        for (var i = 0, len = termList.length; i < len; i++) {
            ...
            tHtml += '<li class="cam-taxpicker-treenode-li">' +
                         '<div class="cam-taxpicker-treenode">' +
                             ...
                             '</div>' +
                             //'<img src="../Images/EMMTerm.png" alt=""/>' +
                             '<img src="' + <%= WebResource("TaxonomyPickerControl.EMMTerm.png")%> + '" alt=""/>' +
    ...
    //TODO: display error message when the control isn't bound correctly????
        ...
        //verify context - No context, commenting out
        //if (!ctx)
        //    return this;
